...
console.log( 'TURN API [ event=%s, datacenter=%s, file=%s ]', 'Loading configuration', env, 'config_'+env+'.js' );

var config = require('./config_'+env);

function getConf(){
  
  console.log( 'TURN API [ event=%s, datacenter=%s, config=%s ]', 'Successfully loaded configuration' , env, JSON.stringify(config) );
